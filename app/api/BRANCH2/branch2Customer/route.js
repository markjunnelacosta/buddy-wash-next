...
export async function GET() {
  await connectToDB();
  const customers = await Branch2Customer.find();
...
export const POST = async (req) => {
  ...
  try {
    await connectToDB();
    const newCustomer = new Branch2Customer({
      customerName,
      customerNumber,
    });
...
export async function DELETE(request) {
  ...
  try {
    ...
    const archivedCustomer = await Branch2Customer.findById(id);
    ...
    await Branch2Customer.findByIdAndDelete(id);
