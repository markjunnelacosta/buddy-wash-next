...
export const GET = async (req, res) => {
  try {
    await connectToDB();
    const dryers = await Branch2Dryer.find({});
...
export const POST = async (req) => {
  ...
  try {
    await connectToDB();
    const newDryer = new Branch2Dryer({
      dryerNumber,
      action,
      timer,
      queue,
...
export async function DELETE(request) {
  ...
  await connectToDB();
  await Branch2Dryer.findByIdAndDelete(id);
...
export async function PATCH(request) {
  ...
  try {
    ...
    await Branch2Dryer.findByIdAndUpdate(id, body);
