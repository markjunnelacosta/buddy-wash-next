...
export async function GET() {
  await connectToDB();
  const supplies = await Branch2Supply.find();
...
export const POST = async (req) => {
  ...
  try {
    await connectToDB();
    const newSupply = new Branch2Supply({
      supplyName,
      availableStock,
      productPrice,
    });
...
export async function DELETE(request) {
  ...
  try {
    ...
    const archivedSupply = await Branch2Supply.findById(id);
    ...
    await Branch2Supply.findByIdAndDelete(id);
