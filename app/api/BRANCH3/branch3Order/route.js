...
import Branch3Order from "@/models/Branch3/Branch3Order";
...
export async function GET() {
  await connectToDB();
  const orders = await Branch3Order.find().populate("machine");
...
export const POST = async (req) => {
  ...
  try {
    ...
    const newOrder = new Branch3Order({
      date,
      name,
      machine: machine._id,
      machineNo,
...
export async function DELETE(request) {
  ...
  await connectToDB();
  await Branch3Order.findByIdAndDelete(id);
...
export async function PATCH(request) {
  ...
  try {
    ...
    await Branch3Order.findByIdAndUpdate(id, body);
