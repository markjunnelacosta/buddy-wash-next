...
const getReport = async () => {
  try {
    ...
    const sortedReportData = response.reportData.sort(
...
const getFilteredReport = async (dateFrom, dateTo, selectedDataPeriod) => {
  try {
    const res = await fetch(
      `/api/BRANCH3/branch3Report?dateFrom=${dateFrom}&dateTo=${dateTo}`,
      {
        cache: "no-store",
      }
    );
    ...
    let filteredData = response.reportData.filter((report) => {
      ...
      return (
        (!dateFrom || reportDate >= new Date(dateFrom)) &&
  ...
  return data.reduce((acc, report) => {
    ...
    switch (selectedDataPeriod) {
      ...
      case "weekly":
        ...
        const diff =
          currentDate.getDate() - dayOfWeek + (dayOfWeek === 0 ? -6 : 1); // Adjust when the day is Sunday
...
const Reports = () => {
  ...
  const handleFilter = async () => {
    try {
      const filteredData = await getFilteredReport(
        dateFrom,
        dateTo,
        selectedDataPeriod
      );
    ...
    try {
      ...
      logoImage.onload = function () {
        ...
        pdf.addImage(
          logoImage,
          "JPEG",
          pdf.internal.pageSize.width - logoSize - 10,
          5,
          logoSize,
          logoSize
        );
        ...
        pdf.setFontSize(10);
        pdf.text(
          "This report contains detailed information about transactions of the business.",
          20,
          45
        ); //specify branch based on account
