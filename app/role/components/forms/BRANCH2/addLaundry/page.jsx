...
const AddLaundry = ({ isOpen, onClose, onSaveData, onUpdateSupply }) => {
  ...
  const onClick = async () => {
    ...
    const orderRes = await fetch("/api/BRANCH2/branch2Order", {
      method: "POST",
      body: JSON.stringify({
        date: orderDate,
        name: customerName,
    ...
    if (detergent && detergentQty) {
      ...
      if (selectedDetergent) {
        const inventoryResponse = await fetch("/api/BRANCH2/branch2Inventory", {
          method: "POST",
          body: JSON.stringify({
            date: dateTime,
        ...
        const res = await fetch(
          `/api/BRANCH2/branch2Supply?id=${selectedDetergent._id}`,
          {
            method: "PATCH",
    ...
    if (fabCon && fabConQty) {
      ...
      if (selectedFabCon) {
        ...
        const res = await fetch(
          `/api/BRANCH2/branch2Supply?id=${selectedFabCon._id}`,
          {
            method: "PATCH",
            body: JSON.stringify({ availableStock: stock }),
            headers: {
