...
import Receipt from "@/app/role/staffBranch3/laundryBin/orderSummary";
...
const AddLaundry = ({ isOpen, onClose, onSaveData, onUpdateSupply }) => {
  ...
  const updateMachineTimer = async (selectedMachine, assignTime) => {
    ...
    const res = await fetch(
      `/api/BRANCH3/branch3Machine?id=${selectedMachine._id}`,
      {
        method: "PATCH",
  ...
  const updateDryerTimer = async (availableDryer) => {
    ...
    // const currDate = new Date();
    const res = await fetch(
      `/api/BRANCH3/branch3Dryer?id=${availableDryer._id}`,
      {
        method: "PATCH",
        body: JSON.stringify({ timer: "tempValue" }),
        headers: {
  ...
  const onClick = async () => {
    ...
    const response = await fetch("/api/BRANCH3/branch3LaundryBin", {
      method: "POST",
      body: JSON.stringify({
    ...
    const orderRes = await fetch("/api/BRANCH3/branch3Order", {
      method: "POST",
      body: JSON.stringify({
        date: orderDate,
        name: customerName,
    ...
    if (detergent && detergentQty) {
      ...
      if (selectedDetergent) {
        const inventoryResponse = await fetch("/api/BRANCH3/branch3Inventory", {
          method: "POST",
          body: JSON.stringify({
            date: dateTime,
        ...
        const res = await fetch(
          `/api/BRANCH3/branch3Supply?id=${selectedDetergent._id}`,
          {
            method: "PATCH",
    ...
    if (fabCon && fabConQty) {
      ...
      if (selectedFabCon) {
        ...
        const res = await fetch(
          `/api/BRANCH3/branch3Supply?id=${selectedFabCon._id}`,
          {
            method: "PATCH",
            body: JSON.stringify({ availableStock: stock }),
            headers: {
  ...
  useEffect(() => {
    const fetchCustomer = async () => {
      try {
        const res = await fetch("/api/BRANCH3/branch3Customer", {
          cache: "no-store",
        });
