...
const AddLaundry = ({ isOpen, onClose, onSaveData, onUpdateSupply }) => {
  const [customerData, setCustomerData] = useState([]); // State for customers
  const [supplyData, setSupplyData] = useState([]); // State for supplies
>>>>>>> b3f645f4829de7be79e17a9414c11da13e90ef1e
  ...
  const onClick = async () => {
    ...
    const selectedDetergent = filterSuppliesByKeyword(
      supplyData,
      "detergent"
    ).find((supply) => supply.supplyName === detergent);
    const selectedFabCon = filterSuppliesByKeyword(
      supplyData,
      "conditioner"
    ).find((supply) => supply.supplyName === fabCon);
